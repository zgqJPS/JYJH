...
统一使用 config.DB_PATH 和 xgt_limit_up_detail / smash_coefficients 表
"""
...
class StrategyTracker:
    ...
    def auto_adjust_parameters(self):
        ...
        for sig_id in range(1, 6):
            ...
            if cf >= 3:
                ...
                new_weight = round(min(weight * 1.2, 3.0), 4)
                new_threshold = round(max(threshold * 0.9, 0.5), 4)
    ...
    def _get_market_data(self, date_str):
        smash = self.conn.execute("""
            SELECT smash_coefficient, max_continuous_days as max_continuous_boards
            FROM smash_coefficients WHERE trade_date = ?
        """, (date_str,)).fetchone()
        total = self.conn.execute("""
