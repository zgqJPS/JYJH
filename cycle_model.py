...
class CycleModel:
    ...
    def detect_phase(self, date_str):
        ...
        try:
            # 获取当日数据
            sc_row = conn.execute(
                "SELECT smash_coefficient, max_continuous_boards FROM smash_coefficient_results WHERE date=? AND smash_coefficient>0",
                (date_str,)
            ).fetchone()
            ...
            prev_row = conn.execute(
                """SELECT s.smash_coefficient, s.max_continuous_boards 
    ...
    def predict_next_day_boards(self, date_str):
        ...
        try:
            ...
            # 砸盘系数修正
            # 砸盘下降→上升概率提升；砸盘上升→下降概率提升
            prev_row = conn.execute(
                """SELECT smash_coefficient FROM smash_coefficient_results 
                   WHERE date < ? AND smash_coefficient > 0 ORDER BY date DESC LIMIT 1""",
                (date_str,)
            ).fetchone()
            ...
            # 修正系数
            if sc_change < -1.5:  # 砸盘明显下降
                adj = 0.15  # 上升概率+15%
            elif sc_change > 1.5:  # 砸盘明显上升
                adj = -0.15  # 上升概率-15%
            else:
                adj = 0
    ...
    def detect_signals(self, date_str):
        ...
        try:
            ...
            # 获取当日和前日数据
            today = conn.execute(
                """SELECT s.smash_coefficient as sc, s.max_continuous_boards as mb,
                          (SELECT COUNT(*) FROM akshare_limit_up WHERE date=s.date) as lu
                   FROM smash_coefficient_results s WHERE s.date=? AND s.smash_coefficient>0""",
                (date_str,)
            ).fetchone()
            ...
            prev2 = conn.execute(
                """SELECT s.smash_coefficient as sc, s.max_continuous_boards as mb
                   FROM smash_coefficient_results s 
                   WHERE s.date < ? AND s.smash_coefficient > 0
                   ORDER BY s.date DESC LIMIT 1 OFFSET 1""",
                (date_str,)
            ).fetchone()
        ...
        try:
            ...
            for i, row in enumerate(rows):
                ...
                if mb <= 3 and sc < 2:
                    ...
                    phase = self.PHASE_ICE  # 冰点但砸盘偏高
                else:
                    phase = self.PHASE_RISE  # 默认蓄力
