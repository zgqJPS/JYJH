...
class PatternRecognizer:
    ...
    def recognize_all(self, date_str, analysis_result):
        ...
        result = {
            ...
            "smash_pattern": self.recognize_smash_pattern(date_str, analysis_result),  # 砸盘系数模式
        }
    ...
    def recognize_dragon(self, date_str, analysis):
        ...
        # 按连板数排序
        sorted_stocks = sorted(stocks, key=lambda x: (x.get("continuous_boards", 1) or 1), reverse=True)
        
        max_board = sorted_stocks[0].get("continuous_boards", 1) or 1 if sorted_stocks else 1
        ...
        for s in sorted_stocks[:5]:
            ...
            if boards >= max(2, max_board - 1):  # 至少次高板
                # 获取概念（从选股宝数据）
    ...
    def _check_dragon_change(self, date_str, dragons):
        ...
        prev_sorted = sorted(prev_stocks, key=lambda x: x.get("continuous_boards", 1) or 1, reverse=True)
    ...
    def recognize_seal_style(self, date_str, analysis):
        ...
        # 风格判断
        if one_pct > 40:
            style = "一字板主导"  # 极端强势，但可能意味着买不到
        elif t_pct > 50:
            style = "T字板为主"  # 有分歧但多头胜出
        elif ex_pct > 50:
            style = "换手板为主"  # 充分换手，分歧较大
        else:
            style = "混合风格"
    ...
    def recognize_market_structure(self, date_str, analysis):
        ...
        has_high = ("高标" in tiers and tiers["高标"].get("count", 0) > 0) or                    ("超高标" in tiers and tiers["超高标"].get("count", 0) > 0)
