...
class SmashCoefficientCalculator:
    ...
    def calculate_daily(self, date):
        """
        计算单日砸盘系数（对比前一交易日）
        返回: (smash_coefficient, max_continuous_boards) 或 (None, None)
        """
        try:
            all_dates = self.db.get_all_dates()
    ...
    def _calc_single_date(self, date, prev_date):
        ...
        计算单日砸盘系数的核心算法
        1. 获取今日和昨日每只股票的连板数
        2. 统计今日和昨日的板级分布（1-10板各有多少只）
        3. 计算晋升比率：今日N板数 / 昨日(N-1)板数（N=2~10）
        4. 取所有有效比率的均值，放大10倍
        """
        try:
            # 获取今日涨停数据
            ...
            # 计算晋升比率：今日N板 / 昨日(N-1)板，N从2到10
            ratios = []
            for n in range(2, 11):
                today_n = today_dist.get(n, 0)    # 今日N板的股票数
                prev_n_minus_1 = prev_dist.get(n - 1, 0)  # 昨日(N-1)板的股票数
