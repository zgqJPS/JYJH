...
def _parse_stock_item(item: Dict, pool_name: str) -> Optional[Dict]:
    """解析单只股票数据 - 适配选股宝API实际返回的字段"""
    try:
        # 提取股票代码（API返回格式：600475.SS → 600475）
        ...
        stock = {
            ...
            'seal_amount': item.get('buy_lock_volume_ratio', 0) or 0,  # 封单占比
            'seal_ratio': item.get('buy_lock_volume_ratio', 0) or 0,   # 封单成交比
...
def save_realtime_to_db(db_path: str, data: Dict[str, Any]) -> int:
    ...
    Args:
        db_path: 数据库路径
        data: fetch_realtime_today()返回的数据
    ...
    try:
        ...
        # 4. 统计概念数据并写入 concept_statistics
        # 从涨停股中提取概念分布
        concept_count = {}
        for stock in limit_up_stocks:
